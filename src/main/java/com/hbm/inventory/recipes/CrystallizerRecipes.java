...
public class CrystallizerRecipes extends SerializableRecipe {
	...
	public void registerDefaults() {
		...
		registerRecipe(P_RED.ore(),		new CrystallizerRecipe(ModItems.crystal_phosphorus, baseTime).prod(0.05F));
		...
		registerRecipe(new ComparableStack(ModItems.crystal_mineral),	new CrystallizerRecipe(ModItems.crystal_diamond, baseTime).prod(0.05F));
		...
		for(FluidStack dye : dyes) {
			registerRecipe(ANY_COAL_COKE.dust(),	new CrystallizerRecipe(DictFrame.fromOne(ModItems.chemical_dye, EnumChemDye.BLACK, 4), mixingTime).prod(0.15F), dye);
